...
async function payMultipleQuotas(socioId){
 let currentSocio=(cuotas.find(x=>String(x.socio_id)===String(socioId))||{}).socios;
 if(!currentSocio){
   const r=await client().from('socios').select('id,numero_socio,nombre_completo,rut,estado,modalidad_cuota').eq('id',socioId).maybeSingle();
   if(r.error)return toast(r.error.message,true);
   currentSocio=r.data;
 }
 if(!currentSocio)return toast('No fue posible identificar al socio.',true);

 const selector=modal(`<h3>Adelantar cuotas</h3>
   ...
   <form>
     ...
     <div class="notice">SIGVE tomará primero cualquier cuota pendiente. Si el socio está al día, creará automáticamente el mes siguiente al último período registrado, aunque corresponda al año siguiente.</div>
