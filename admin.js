...
$('directiva-form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f));d.activo=f.activo.checked;d.orden=Number(d.orden||1);d.nombre=normalizeName(d.nombre);d.cargo=cleanSpaces(d.cargo);d.telefono=cleanSpaces(d.telefono);d.descripcion=cleanSpaces(d.descripcion);const id=d.id;delete d.id;d.actualizado_por=state.user.id;const q=id?sb.from('directiva').update(d).eq('id',id):sb.from('directiva').insert(d);const{error}=await q;if(error)return message(error.message,true);message('Integrante guardado.');resetDirectiva();loadDirectiva()};$('cancel-directiva').onclick=resetDirectiva;
