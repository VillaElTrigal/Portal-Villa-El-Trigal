...
async function loadAll(){for(const t of Object.keys(defs))await loadTable(t);await loadConfig();await loadDirectiva();await loadDocumentos()}
...
buildForms();bindMultiInputs();document.querySelectorAll('form[data-table]').forEach(f=>{f.onsubmit=async e=>{e.preventDefault();try{await saveForm(f)}catch(err){message(err.message||String(err),true)}};f.querySelector('.cancel').onclick=()=>resetForm(f)});sb.auth.onAuthStateChange(event=>{if(event==='PASSWORD_RECOVERY'){$('login-form').hidden=true;$('recovery-form').hidden=false}else setTimeout(verify,0)});verify();
})();
