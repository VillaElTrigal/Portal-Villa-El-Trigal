...
const validRut=(value)=>{const clean=rutClean(value);if(clean.length<7)return false;const body=clean.slice(0,-1),dv=clean.slice(-1);let sum=0,multiplier=2;for(let i=body.length-1;i>=0;i--){sum+=Number(body[i])*multiplier;multiplier=multiplier===7?2:multiplier+1}const result=11-(sum%11),expected=result===11?'0':result===10?'K':String(result);return dv===expected};
  ...
  async function loadStreetWhatsapp(){const card=$('streetWhatsappCard'),link=$('streetWhatsappLink');if(!card||!link)return;card.hidden=true;try{const rows=await rpc('portal_socio_grupo_whatsapp',{p_token:token}),g=rows?.[0];if(!g?.enlace_invitacion)return;$('streetWhatsappName').textContent=g.nombre_grupo||'Grupo de tu calle';$('streetWhatsappText').textContent=`Grupo correspondiente a ${g.via_nombre||'tu calle'}. Únete para mantenerte comunicado con tus vecinos.`;link.href=g.enlace_invitacion;card.hidden=false}catch(e){console.warn('Grupo WhatsApp no disponible',e)}}
  ...
  $('telefono')?.addEventListener('input',()=>{$('telefono').value=String($('telefono').value||'').replace(/\D/g,'').slice(0,8)});
  ...
  async function enter(){try{if(!vias.length)await loadVias();if(!viaPicker)viaPicker=bindViaPicker();const rows=await rpc('portal_socio_mis_datos',{p_token:token});const s=rows?.[0];if(!s)throw new Error();currentSocio=s;currentMemberNumber=normalizeMemberNumber(s.numero_socio);try{const m=(await rpc('portal_socio_modalidad',{p_token:token}))?.[0];portalModality=m?.modalidad_cuota||'cotizante';currentSocio._modalidad=m}catch(_){portalModality='cotizante'}renderBirthdayBanner(s);$('loginView').hidden=true;$('portalView').hidden=false;$('logout').hidden=false;$('welcomeName').textContent=s.nombre_completo;$('numero').textContent=s.numero_socio||'—';$('rutRead').textContent=formatRut(s.rut);$('fechaIngreso').textContent=fmtDate(s.fecha_ingreso);$('estado').textContent=s.estado;$('nombres').value=s.nombres||'';$('apellidoPaterno').value=s.apellido_paterno||'';$('apellidoMaterno').value=s.apellido_materno||'';$('fechaNacimiento').value=s.fecha_nacimiento||'';$('edad').value=calcAge(s.fecha_nacimiento);$('estadoCivil').value=s.estado_civil||'';$('ocupacion').value=s.ocupacion||'';$('ocupacionOtro').value=s.ocupacion_otro||'';syncOccupation();viaPicker.set(s.via_id,s.numero_domicilio,s.direccion);$('telefono').value=phoneDigitsPortal(s.telefono||'');$('correo').value=s.correo||'';if($('portalCertName')){$('portalCertName').value=s.nombre_completo||'';$('portalCertNationality').value=s.nacionalidad||'';$('portalCertRut').value=formatRut(s.rut);$('portalCertAddress').value=s.direccion||'';$('portalCertPhone').value=formatPhonePortal(s.telefono||'');$('portalCertEmail').value=s.correo||'';syncPortalCertRecipient();}const pending=Number(s.cuotas_pendientes||0);$('duesStatus').textContent=portalModality==='asociado'?'No aplica · Socio asociado':(pending?'Pendiente':'Al día');$('duesStatus').classList.toggle('pending',pending>0);$('lastPaid').textContent=s.ultima_cuota_pagada?fmtDate(s.ultima_cuota_pagada):'Sin pagos registrados';$('pendingCount').textContent=pending;$('pendingAmount').textContent=money(s.monto_adeudado);await loadChildren();await loadPortalQuotas();await loadPortalBenefits();await loadStreetWhatsapp();await loadAssociatedPanel()}catch(e){console.error(e);sessionStorage.removeItem('sigve_portal_token');token=null;$('loginView').hidden=false;$('portalView').hidden=true;$('logout').hidden=true;msg('authMsg','Tu sesión venció o no fue posible cargar tus datos. Ingresa nuevamente.','error')}}
  $('dataForm').addEventListener('submit',async(e)=>{e.preventDefault();const address=viaPicker.get();if(!address){msg('dataMsg','Selecciona una calle, pasaje o avenida válida e ingresa el número.','error');return}if($('ocupacion').value==='Otro'&&!$('ocupacionOtro').value.trim()){msg('dataMsg','Especifica tu ocupación.','error');return}const phoneDigits=String($('telefono').value||'').replace(/\D/g,'').slice(0,8);const phone=phoneDigits.length===8?`+569${phoneDigits}`:null;if(phoneDigits.length>0&&phoneDigits.length!==8){msg('dataMsg','Ingresa exactamente los 8 dígitos después de +56 9.','error');$('telefono').focus();return}msg('dataMsg','Guardando…');try{await rpc('portal_socio_actualizar_datos',{p_token:token,p_nombres:$('nombres').value.trim(),p_apellido_paterno:$('apellidoPaterno').value.trim(),p_apellido_materno:$('apellidoMaterno').value.trim(),p_fecha_nacimiento:$('fechaNacimiento').value,p_estado_civil:$('estadoCivil').value,p_ocupacion:$('ocupacion').value,p_ocupacion_otro:$('ocupacion').value==='Otro'?$('ocupacionOtro').value.trim():null,p_via_id:address.via_id,p_numero_domicilio:address.numero_domicilio,p_telefono:phone,p_correo:$('correo').value});$('welcomeName').textContent=[$('nombres').value.trim(),$('apellidoPaterno').value.trim(),$('apellidoMaterno').value.trim()].filter(Boolean).join(' ');msg('dataMsg','Datos actualizados correctamente.','success')}catch(err){msg('dataMsg',err.message,'error')}});
  ...
  async function loadPortalBenefits(){
    ...
    if(portalModality==='asociado'){box.innerHTML='<div class="notice">Tu modalidad es <strong>Socio asociado</strong>. No generas beneficios económicos independientes por pago de cuotas.</div>';return}
    try{
  ...
  async function loadAssociatedPanel(){
    const box=$('associatedEligibility'),form=$('associatedForm'),list=$('associatedMembersList');
    if(!box||!form)return;
    ...
    if(list){
      list.innerHTML='<div class="muted">Cargando socios asociados…</div>';
      try{
        const rows=await rpc('portal_socio_mis_asociados',{p_token:token});
        if(!rows?.length){
          list.innerHTML='<div class="muted">Actualmente no tienes socios asociados vinculados.</div>';
        }else{
          list.innerHTML=`<h3>Socios asociados a mi hogar</h3>${rows.map(x=>`
        ...
        list.innerHTML='<div class="muted">No fue posible cargar los socios asociados.</div>';
      }
    }
  }
