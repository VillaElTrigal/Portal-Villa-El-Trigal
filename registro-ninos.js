(() => {
  ...
  document.querySelectorAll('[data-renew-method]').forEach(button=>button.addEventListener('click',async()=>{
    ...
    const phone=String(result.telefono||'');const wa=window.SIGVEWhatsApp?.buildUrl(phone,text)||'';const mail=result.correo?`mailto:${encodeURIComponent(result.correo)}?subject=${encodeURIComponent('Nuevo enlace para registro de niños y niñas')}&body=${encodeURIComponent(text)}`:'';
