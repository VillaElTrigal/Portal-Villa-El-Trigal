...
async function openCert(x=null){
  const [st,so,folioRes]=await Promise.all([
    sb.from('configuracion_gestion').select('*').eq('id',1).single(),
    sb.from('socios').select('id,numero_socio,nombre_completo,rut,nacionalidad,direccion,telefono,correo').eq('estado','activo').order('nombre_completo'),
    x?Promise.resolve({data:x.folio}):sb.from('certificados_emitidos').select('folio').order('folio',{ascending:false}).limit(1).maybeSingle()
  ]);
...
async function printCert111(x){
  ...
  const firma=(a,cargo,mostrar=true)=>`<div class="cert-signature">${mostrar&&a?.firma_data_url?`<img src="${a.firma_data_url}" alt="Firma ${cargo}">`:''}<div class="line">${a?.nombre_completo?`<strong>${esc(a.nombre_completo)}</strong><br>`:''}${cargo}</div></div>`;
  const timbre=conf.timbre_data_url?`<img class="cert-stamp-img" src="${conf.timbre_data_url}" alt="Timbre oficial">`:`<div class="cert-stamp-circle">TIMBRE<br>OFICIAL</div>`;
  ...
  w.document.write(`<html><head><meta charset="utf-8"><title>Certificado ${x.folio}</title><link rel="stylesheet" href="admin-v111.css?v=565"></head><body>
  <div class="cert-print-sheet cert-residencia-oficial">
    ...
    <div class="cert-signatures cert-signatures-photo">
      ${firma(secretario,'Secretario(a)',conf.firma_secretario!==false)}
      <div class="cert-stamp-space">${timbre}<small>Junta de Vecinos Villa El Trigal</small></div>
      ${firma(presidente,'Presidente(a)',conf.firma_presidente!==false)}
    </div>
...
async function loadAuthorities(){const{data,error}=await sb.from('autoridades_junta').select('*').order('activo',{ascending:false}).order('cargo');if(error)return toast(error.message,true);$('#authorities-list').innerHTML=(data||[]).map(a=>`<div class="authority-card"><div><strong>${esc(a.nombre_completo)}</strong> · ${esc(a.cargo)}${a.activo?' · Vigente':' · Histórico'}<small>${a.periodo_desde?dateCL(a.periodo_desde):'Sin fecha'} — ${a.periodo_hasta?dateCL(a.periodo_hasta):'Actual'}</small>${a.firma_data_url?`<img class="signature-preview" src="${a.firma_data_url}">`:''}</div><div><button class="button secondary" data-edit-authority="${a.id}">Editar</button></div></div>`).join('')||'<p>No hay autoridades registradas.</p>';$$('[data-edit-authority]').forEach(b=>b.onclick=()=>editAuthority((data||[]).find(a=>a.id===b.dataset.editAuthority)));const{data:conf}=await sb.from('configuracion_documentos').select('*').eq('id',1).single();if(conf){const f=$('#document-config-form');f.certificado_digital_habilitado.checked=conf.certificado_digital_habilitado;f.firma_presidente.checked=conf.firma_presidente;f.firma_secretario.checked=conf.firma_secretario;f.firma_tesorero.checked=conf.firma_tesorero;
    const wrap=$('#stamp-preview-wrap'), img=$('#stamp-preview');
    if(wrap&&img&&conf.timbre_data_url){img.src=conf.timbre_data_url;wrap.hidden=false}else if(wrap){wrap.hidden=true}}
}
...
function bind(){const nav=$('.sidebar nav');if(nav&&!nav.querySelector('[data-section="autoridades"]')){const directiva=nav.querySelector('[data-section="directiva"]');directiva?.insertAdjacentHTML('afterend','<button data-section="autoridades">✍️ Autoridades y firmas</button>')}$('#new-certificado').textContent='＋ Registrar certificado manual';$('#new-certificado').onclick=()=>openCert();$('#cert-search').oninput=renderCerts111;$('#new-authority').onclick=()=>editAuthority();$('#document-config-form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget;
  let timbre=null;try{timbre=await fileData(f.timbre?.files?.[0])}catch(err){return toast(err.message,true)}
  const cambios={certificado_digital_habilitado:f.certificado_digital_habilitado.checked,firma_presidente:f.firma_presidente.checked,firma_secretario:f.firma_secretario.checked,firma_tesorero:f.firma_tesorero.checked,actualizado_en:new Date().toISOString()};
  if(timbre)cambios.timbre_data_url=timbre;
  const{error}=await sb.from('configuracion_documentos').update(cambios).eq('id',1);
  if(error)return toast(error.message,true);toast('Configuración de documentos guardada.');loadAuthorities()};document.addEventListener('click',e=>{const b=e.target.closest('[data-section]');if(!b)return;if(b.dataset.section==='autoridades'){e.stopImmediatePropagation();$$('[data-section]').forEach(x=>x.classList.toggle('active',x===b));$$('.admin-section').forEach(s=>s.hidden=s.id!=='section-autoridades');$('#page-title').textContent='Autoridades y firmas';loadAuthorities()}if(b.dataset.section==='certificados')setTimeout(loadCerts111,80)},true)}
