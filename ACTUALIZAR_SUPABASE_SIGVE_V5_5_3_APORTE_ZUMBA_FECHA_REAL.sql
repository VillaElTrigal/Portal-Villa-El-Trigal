-- SIGVE v5.5.3 · Aporte mensual de Zumba con período y fecha de pago independientes
...
begin
  ...
  if p_cantidad_clases is null or p_cantidad_clases <= 0 then
    raise exception 'La cantidad de clases debe ser mayor que cero';
  end if;
  if p_valor_clase is null or p_valor_clase <= 0 then
  ...
  insert into public.zumba_pagos(
    ...
    v_monto,
    p_medio,
    coalesce(nullif(btrim(p_observaciones),''),p_cantidad_clases || ' clases a $' || trim(to_char(p_valor_clase,'FM999G999G990')) || ' cada una'),
    auth.uid()
